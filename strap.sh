    ...
    (
      ...
      for i in "$@"; do
        if [ -f "$i" ] && [ -x "$i" ]; then
          ...
          if [ -z "$STRAP_DEBUG" ]; then
            "STRAP_GIT_NAME=$STRAP_GIT_NAME STRAP_GIT_EMAIL=$STRAP_GIT_EMAIL STRAP_GITHUB_USER=$STRAP_GITHUB_USER $i" 2>/dev/null
          else
            "STRAP_GIT_NAME=$STRAP_GIT_NAME STRAP_GIT_EMAIL=$STRAP_GIT_EMAIL STRAP_GITHUB_USER=$STRAP_GITHUB_USER $i"
          fi
          break
        fi
      done
...
# Get user GitHub information - this is not embedded in the script so it can be checked in!
userinput 'What is your GitHub author name?'
read -e STRAP_GIT_NAME
userinput 'What is your GitHub author email?'
read -e STRAP_GIT_EMAIL
userinput 'What is your GitHub user?'
read -e STRAP_GITHUB_USER
userinput 'What is your GitHub access token?'
read -e STRAP_GITHUB_TOKEN
...
if [ -n "$STRAP_GITHUB_USER" ]; then
  ...
  if git ls-remote "$DOTFILES_URL" &>/dev/null; then
    ...
    run_dotfile_scripts ~/.dotfiles/script/bootstrap
    logk
  fi
fi

# Setup Brewfile
if [ ! -f "$HOME/.Brewfile" ] && [ -f "$HOME/.dotfiles/Brewfile" ]; then
